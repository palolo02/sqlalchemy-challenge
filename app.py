...
def precipitation():
    try:
        ...
        precipitation = session.query(*sel).filter(Measurement.date >= '2016-08-23').filter(Measurement.date <= last_date.c.last_date).group_by(Measurement.date).order_by(Measurement.date).all()
...
def temperature():
    try:
        ...
        waihee_data = session.query(*sel_waihee_data).filter(Measurement.station == 'USC00519281').filter(Measurement.date >= '2016-08-23').filter(Measurement.date <= last_date.c.last_date).group_by(Measurement.date).order_by(Measurement.date).all()
        # Transform into dataframe
...
def range_temperature(start,end):
    ...
    session = Session(engine)
    sel_list_temp = [Measurement.date, func.min(Measurement.tobs).label('min_temp'), func.avg(Measurement.tobs).label('avg_temp'), func.max(Measurement.tobs).label('max_temp') ]
    temp = session.query(*sel_list_temp).filter(Measurement.date >= start).filter(Measurement.date <= end).group_by(Measurement.date).order_by(Measurement.date).all()
    # Transform into dataframe
    temp_df = pd.DataFrame(temp, columns=["day","Min Temp","Avg Temp","Max Temp"])
    # update datatype for day
    temp_df["day"] = pd.to_datetime(temp_df["day"])
